    ...
    if _CSV_DATA is None:
        try:
            ...
            if 'url' in _CSV_DATA.columns:
                _CSV_DATA['normalized_csv_url'] = _CSV_DATA['url'].apply(_normalize_url_for_matching)
            else:
                print(f"Warning: 'url' column not found in CSV: {_CSV_FILE_PATH}")
                _CSV_DATA = pd.DataFrame()
            print(f"Successfully loaded and preprocessed CSV data from: {_CSV_FILE_PATH}")
...
def url_detector_1(url: List[str]) -> List[int]:
    ...
    if csv_data.empty or 'normalized_csv_url' not in csv_data.columns:
        print("Warning: CSV data is empty or 'normalized_csv_url' column is missing. Defaulting all to malicious.")
        return [1] * len(url)
...
def url_detector_4(url: List[str], timeout=30, fetch_method='requests') -> List[int]:
    ...
    for idx, u in enumerate(url):
        ...
        if fetch_method == 'selenium':
            ...
            try:
                response = requests.get(u, timeout=(timeout / 2))
...
def return_tools():
    ...
    tools = [
        ...
        StructuredTool(
            ...
            description='''
            This tool accepts a list of URLs and evaluates their safety using an XGBoost model deployed locally. 
